...
function renderPlaylist(){
  ...
  state.playlist.forEach((t,i) => {
    ...
    el.className = 'track' + (i===state.currentIndex ? ' active' : '');
    el.tabIndex = 0;
    
    // Constraints added to inner divs to prevent horizontal scrollbars
    el.innerHTML = `<div style="flex-shrink:0;width:36px;height:36px;border-radius:6px;background:linear-gradient(180deg, rgba(255,255,255,0.02),transparent);display:grid;place-items:center;border:1px solid rgba(255,255,255,0.02)">${i+1}</div>
                    <div style="min-width:0; flex-grow:1; overflow:hidden;">
                      <div style="font-weight:600; font-size:14px; color: #e9fbff; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${escapeHtml(t.title || t.src.split('/').pop() || 'Untitled')}</div>
                      <div style="font-size:12px; color:rgba(235,245,255,0.7); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${escapeHtml(t.artist || '')}</div>
                    </div>`;
  ...
  // Autoscroll logic (scrollTo) has been completely removed to respect user interaction
}
